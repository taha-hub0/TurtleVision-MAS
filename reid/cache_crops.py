...
def _crop_one(args):
    image_root, out_dir, row, size, margin = args
    ...
    if hw and hw > 0:
        ...
        box, used = (0, 0, int(w), int(h)), 'full'

    try:
        src = os.path.join(image_root, str(file_name).replace('/', os.sep))
        with Image.open(src) as im:
            im = im.convert('RGB').crop(box).resize((size, size), Image.BILINEAR)
...
def main():
    ...
    tasks = [(args.image_root, args.out_dir, r, args.size, args.margin) for r in rows]

    print(f'{len(tasks)} kirpma, {args.workers} surec, hedef {args.out_dir}')
    counts, done = {}, 0
